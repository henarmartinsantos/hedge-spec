...
5. Data subject: `ex:user18` vs `dpv:User`
        
"Access by non-healthcare professional as data controller to specific subset odata (e.g. lab test results of one specific blood sampling, like a glucose testfor 24h."
        
The assigner and DataSubject is a specific user, so `ex:userXX` will be used:
```
        odrl:assigner ex:user18 ;
        dpv:DataSubject ex:user18 ;
```